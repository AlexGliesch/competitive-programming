...
void backtrack(long long lit, long long lamps, int num_lamps, int r, int c) {
  ...
  if (ok(r - 1, c - 1) and bar[r - 1][c - 1] >= 0 and
      around[r - 1][c - 1] != bar[r - 1][c - 1])
    return;

  backtrack(lit, lamps, num_lamps, (c == C - 1) ? r + 1 : r,
            (c == C - 1) ? 0 : c + 1);

  if (bar[r][c] != -100 or (lit & ix(r, c)))
    return;
  ...
  backtrack(lit, lamps | ix(r, c), 1 + num_lamps, (c == C - 1) ? r + 1 : r,
            (c == C - 1) ? 0 : c + 1);
